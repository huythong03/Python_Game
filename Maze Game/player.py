...
class Player:
    ...
    #Nhận vị trí ô hiện tại của người chơi
    def get_current_cell(self, x, y, grid_cells):
        for cell in grid_cells:
            if cell.x == x and cell.y == y:
                return cell
            
    #Dừng người chơi đi qua tường
    def check_move(self, tile, grid_cell, thickness):
        current_cell_x, current_cell_y = self.x // tile, self.y // tile
        current_cell = self.get_current_cell(current_cell_x, current_cell_y, grid_cell)
    ...
    #Vẽ trình phát lên màn hình
    def draw(self, screen):
        pygame.draw.rect(screen, self.color, self.rect)
    
    #Cập nhập vị trí người chơi trong khi di chuyển
    def update(self):
        self.velX = 0
        self.velY = 0
